...
export const PurchaseProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const storeKitConnectedRef = useRef(false);
  ...
  useEffect(() => {
    ...
    const initStoreKit = async () => {
      ...
      try {
        ...
        if (cancelled) {
          await InAppPurchases.disconnectAsync().catch(console.error);
          return;
        }

        storeKitConnectedRef.current = true;
        setIsConnected(true);
    ...
    return () => {
      cancelled = true;
      if (storeKitConnectedRef.current) {
        storeKitConnectedRef.current = false;
